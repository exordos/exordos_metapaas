...
# anonymous reads; leave it out of skip_auth so the middleware creates an
# AnonDriver context and the policy check can proceed normally.
skip_auth_endpoints = [
    iam_mw.EndpointComparator(r"/v1/"),
]
...
def get_openapi_engine():
    openapi_engine = openapi_engines.OpenApiEngine(
        info=openapi_structures.OpenApiInfo(
            title=f"Exordos MetaPaaS {versions.API_VERSION_1_0} User API",
            version=app_version.version_info,
            description=(f"OpenAPI - Exordos MetaPaaS {versions.API_VERSION_1_0}"),
        ),
